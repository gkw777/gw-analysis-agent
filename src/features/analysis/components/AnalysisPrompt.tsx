// 경영 분석 프롬프트 화면 — PromptLayout 셸 위에 실제 입력 상태와 메시지 목록을 조립한 구현체.
// splitView 전환 시 AnalysisPage.tsx 의 분기 구조가 바뀌면서 이 컴포넌트가 리마운트되므로,
// messages 는 로컬 상태가 아닌 messagesAtom 으로 관리해 화면 전환 후에도 유지되게 한다.
// 백엔드(services/) 연동 전 단계이므로 전송 시 atom 에만 반영하며, 이후 실제 분석 API 연동 시
// handleSubmit 내부만 교체하면 된다. 첫 메시지 전송 시 우측 보고서 분할뷰를 연다 — 실제 분석 API 연동 후에는
// 이 트리거를 "보고서가 생성되었을 때"로 교체한다.
